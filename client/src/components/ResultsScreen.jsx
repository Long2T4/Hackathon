...
import { ArrowLeft, MapPin, Phone, Globe, Star, Navigation, Loader2, MessageCircle, RotateCcw, FileText } from 'lucide-react'
...
function ClinicItem({ clinic, index, lang, urgency }) {
  ...
  const costHint = urgency === 'emergency'
    ...
    ? { es: 'Opción más asequible para pagos directos', en: 'Most affordable option for self-pay', color: 'text-green-700 bg-green-50' }
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start gap-2 flex-wrap">
            ...
            {index === 0 && <span className="text-[10px] font-bold bg-violet-100 text-violet-600 px-2 py-0.5 rounded-full">{t('Mejor opción', 'Best Match')}</span>}
          </div>
          ...
          <div className="flex gap-2 mt-3">
            ...
            <a href={directionsUrl} target="_blank" rel="noopener noreferrer" className="flex-1 flex items-center justify-center gap-1 border border-gray-200 rounded-xl py-2 text-xs font-semibold text-gray-600"><Globe size={11} /> {t('Cómo llegar', 'Directions')}</a>
            <a href={`tel:`} className="flex-1 flex items-center justify-center gap-1 bg-violet-600 rounded-xl py-2 text-xs font-bold text-white"><Phone size={11} /> {t('Llamar', 'Call')}</a>
          </div>
        </div>
      </div>
    </div>
...
export default function ResultsScreen({ lang, patientData, symptomsData, analysisData, onBack, onChat, onRestart }) {
  const t = (es, en) => lang === 'es' ? es : en
  ...
  useEffect(() => {
    navigator.geolocation?.getCurrentPosition(async (pos) => {
      ...
      } catch { setError(t('No se pudieron cargar las clínicas.', 'Could not load clinics.')) }
      setLoading(false)
    }, () => { setError(t('Activa tu ubicación para ver clínicas cercanas.', 'Enable location to see nearby clinics.')); setLoading(false) })
  }, [])
  ...
  const handlePDF = () => {
    ...
    try {
      generateVisitCardPDF({
        clinicianSummary: analysisData?.clinicianSummary || analysisData?.summary || '',
        symptomsES: symptomsData?.text || '',
        symptomsEN: symptomsData?.text || '',
        assessmentES: analysisData?.summary || '',
        assessmentEN: analysisData?.summary || '',
        urgency: analysisData?.urgency || 'routine',
        questionsES: analysisData?.questions || [],
        questionsEN: analysisData?.questions || [],
      })
    ...
    <div className="flex-1 flex flex-col overflow-hidden">
      <div className="px-4 py-3 bg-white border-b border-gray-100">
        <div className="flex items-center justify-between mb-2">
          <p className="text-xs text-gray-500 font-medium">{t('Paso 4 de 4', 'Step 4 of 4')}: {t('Clínicas', 'Clinics')}</p>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-6">
        ...
        <button onClick={handlePDF} disabled={generatingPDF} className="w-full bg-white border border-gray-200 rounded-2xl p-4 flex items-center gap-3 mb-3 shadow-sm">
          <div className="w-10 h-10 bg-orange-50 rounded-xl flex items-center justify-center">
            <FileText size={18} className="text-orange-500" />
          </div>
          <div className="flex-1 text-left">
            <p className="text-sm font-semibold text-gray-900">{t('Generar Tarjeta de Visita', 'Generate Visit Card')}</p>
            <p className="text-xs text-gray-400">{t('PDF bilingüe para llevar al doctor', 'Bilingual PDF to bring to doctor')}</p>
          </div>
